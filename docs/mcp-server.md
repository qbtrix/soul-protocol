<!-- Covers: MCP server setup, configuration for Claude Desktop/Cursor, all 27 tools
     (14 soul/memory + 5 context + 5 psychology + 3 trust chain), 3 resources, 2 prompts,
     auto-detect, MCP Sampling Engine, programmatic usage, and design notes.
     Updated: 2026-04-29 — v0.5.0 (#203): Added soul_prune_chain MCP tool for touch-time
     chain pruning. Dry-run by default; pass apply=true to mutate the chain. Tool count: 26 → 27.
...
All tools are prefixed `soul_` to avoid name collisions when running alongside other MCP servers. The tools break down as: 9 soul management, 5 memory, 5 context (LCM), 5 psychology pipeline (v0.2.7), and 3 trust chain (`soul_verify`, `soul_audit`, `soul_prune_chain`).
