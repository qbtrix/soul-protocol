...
- **Episodic plain-content writes no longer get the `User:/Agent:` envelope (#234)** — `Soul.remember(text, type=EPISODIC)` and `Soul.note(text, type=EPISODIC)` now store content verbatim. The previous path in `MemoryManager.add()` fabricated an `Interaction(user_input=text, agent_output="")` and routed it through `EpisodicStore.add()`, which formatted it as `User: <text>\nAgent: ` with a trailing empty `Agent:`. The envelope is correct when the caller has real user/agent inputs (the `soul observe` pipeline keeps using `add()` / `add_with_psychology()`), but plain writes from shell hooks and scripts ended up with a fake chat exchange that never happened. New path: `EpisodicStore.add_entry(MemoryEntry)` stores a pre-formed entry without reshaping content. The `soul-sync.sh` regex strip workaround in paw-workspace can drop once this release ships.
