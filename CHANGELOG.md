...
### Graph traversal + typed entity ontology (#108, #190)

- **Typed entity ontology.** Entities now carry one of eight built-in kinds (`person`, `place`, `org`, `concept`, `tool`, `document`, `event`, `relation`) plus open-string extension. Custom strings (`pr`, `channel`, `library`, `issue`) pass through untouched — `EntityType` just names the well-known ones. Eight matching relation predicates (`mentions`, `related`, `depends_on`, `contributes_to`, `causes`, `follows`, `supersedes`, `owned_by`) ship as `RelationType` with the same open contract.
- **Real LLM-driven extraction.** The cognitive engine's `extract_entities` prompt now asks for the typed ontology plus a `relations` array per entity with `{target, relation, weight}` triples. Heuristic-only souls keep working — a translation table maps legacy heuristic types to the new ontology so engine-less callers still produce typed entities. Re-extraction on the same interaction is idempotent: identical `(source, target, relation)` triples are deduped while provenance lists pick up the second memory's id. Contract documented in `extract_entities` docstring.
- **`Soul.graph`.** New top-level property returning a `GraphView` with `nodes()`, `edges()`, `neighbors()`, `path()`, `subgraph()`, `to_mermaid()`, `reachable()`, `stats()`. The view is the canonical read surface; mutations land via `Soul.observe()` so trust chain entries get appended.
- **Recall graph_walk + progressive loading.** `Soul.recall` accepts three new keyword arguments. `graph_walk={"start": entity_id, "depth": 2, "edge_types": ["mentions"]}` filters results to memories linked to entities reachable from the starting node, ranked by combined relevance + graph distance. `page_token` resumes a previous walk (the token is bound to the original `(query, graph_walk)` pair — mismatched tokens raise `ValueError`). `token_budget` caps cumulative content size; overflow entries fall back to their L0 abstract via the F1 progressive disclosure mechanism. New `RecallResults` (a `list` subclass) carries `next_page_token`, `total_estimate`, `truncated_for_budget` attributes when these features are in use; legacy callers that just pass a query keep getting a plain `list[MemoryEntry]`.
- **Trust chain hooks.** `Soul.observe()` now appends `graph.entity_added` and `graph.relation_added` entries for net-new entities/edges. Payloads are compact (id/type/source) — never full content — matching the existing `memory.write` rule.
- **CLI: `soul graph` subcommand group.** `soul graph nodes <path>` (with `--type`/`--match`/`--limit`/`--json`), `soul graph edges <path>` (with `--source`/`--target`/`--relation`/`--json`), `soul graph neighbors <path> <node_id>` (with `--depth`/`--types`/`--json`), `soul graph path <path> <source> <target>` (with `--max-depth`/`--json`), `soul graph mermaid <path>`. All commands honor `--json` for machine-readable output.
- **MCP: `soul_graph_query`.** Discriminated query tool — `kind=nodes|edges|neighbors|path|subgraph|mermaid|stats` — exposes the GraphView over MCP. Tool count: 26 -> 27.
- **Storage model.** In-memory dict + adjacency list. Entities stored as `name -> type` plus a parallel `_provenance` map (`name -> [memory_id]`). Edges stay as a `list[TemporalEdge]` with weight + metadata. Round-trips through the existing `to_dict`/`from_dict` JSON path. Pre-0.5.0 graphs load cleanly — `provenance` is optional on disk.
- **Heuristic extractor improvements.** Entity-to-entity edges (third-person relations like "Alice knows Bob") now flow through to the graph instead of being dropped. The previous `Soul.observe` code only forwarded the user-relation edge, leaving the third-person `relationships` field unread.
