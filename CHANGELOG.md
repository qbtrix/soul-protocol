...
- **Migration:** Existing souls exported before this change have skills with no `source` field on disk. On awaken, these skills restore with `source=None` and are treated as non-public. To re-advertise a skill, call `soul.skills.add(Skill(id=..., name=..., source=SkillSource.MANUAL))` — the `add()` collision path upgrades `source` to `MANUAL` on existing entries. This is the intended trade-off: erring private until explicitly re-registered.
- **SDK note:** `SkillRegistry.add()` without an explicit `source` now creates a non-public skill (source defaults to `None`). Callers that intend skills to appear in `public_profile()` or A2A cards must pass `source=SkillSource.MANUAL`.
- Added `SkillSource` enum (`MANUAL`, `ENTITY`, `PROCEDURE`, `LEARNING`) to `skills.py`.
- Added `Skill.source` field (defaults to `None` for backward compat).
- Added `SkillRegistry.public_skills()` filter method.

### Dedup-aware memory writes
...
### Memory Schema Consolidation (#285)

- **Breaking:** `MemoryEntry` construction with only `content` now raises `ValueError`. Callers must provide either `type` or a non-empty `layer`.
- **Breaking:** `MemoryEntry.id` now defaults to a random 12-character hex string (e.g. `"a1b2c3d4e5f6"`) instead of `""`. Any sentinel logic checking `if not entry.id:` must be updated.
...
- **`soul diff` structured comparison (#191)** — read-only command compares two soul files at the soul level, not the byte level. Sections covered: identity, OCEAN/DNA, state, core memory, memories per layer + per domain, bond (default + per-user), skills, trust chain, self-model, evolution. Memory diff is by id: added, removed, modified. Output formats: text (default Rich panel, sections omitted when empty), `--format json` (full `SoulDiff` Pydantic dump for tooling), `--format markdown` (paste-ready table for PR bodies). Flags: `--section <name>` narrows to one section (accepts `dna`/`ocean`, `core`/`core-memory`, etc.); `--include-superseded` surfaces the supersession chain explicitly; `--summary-only` collapses to per-section counts. Schema-version mismatch raises a clean error pointing at `soul migrate`. New public Python API: `soul_protocol.runtime.diff_souls`, `SoulDiff`, `SchemaMismatchError`. Useful for team-soul PR review (does the new memory I'm promoting carry the wording I think it does?) and migration verification (did the corrected fact actually propagate?).
